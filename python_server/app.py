...
# 모델 전역 로드
models = load_models()
logistic_model, xgb_model, meta_model, encoder = models
...
def get_pitcher_stats(year):
    try:
        ...
        print(f"투수 통계 조회 실패: {str(e)}")
        return pd.DataFrame()

# ===== 피처 엔지니어링 (realtime_predict.py와 동일) =====
...
def predict():
    try:
        ...
        if not date:
            return jsonify({"error": "날짜가 필요합니다 (YYYY-MM-DD)"}), 400
